...
bertX <- tm_shape(zx) + tm_polygons(fill="Ag40", fill.scale = tm_scale(values=c("white", "orange3")), fill.legend = tm_legend_hide()) + tm_shape(tt1_geo) + tm_dots(size=0.4) + tm_text("name", options = opt_tm_text(just="left"), col="blue", xmod=0.5)
bertX
```

Figure \@ref(fig:Bertagp) shows the $90$ departments of France in 1954 with the highlighted in which agricultural workers made up at least $40\%$ of the workforce.  There is a fairly clear division into East and West, something that is described in French texts under the heading of the Le Havre-Marseille line.  The line may have declined in importance over the last thirty years, but in 1954 it was still relevant.\index{map}
...
The total numbers of workers in the three sectors are of the same order (Commerce  million, Industry  million, Agriculture  million).  The distributions for both Industry and Commerce are highly skewed\index{skewness} due to a handful of big values in both, primarily Paris and Seine.  The distribution of agricultural workers is less skewed.  No department has more than the  agricultural workers of in the furthest North West of France.
...
tm_shape(zx) + tm_polygons(fill="Agriculture", fill.scale=tm_scale(values="brewer.yl_or_br"), fill.legend = tm_legend(position = c("left", "bottom"))) + tm_shape(tt2_geo) + tm_dots(size=0.4) + tm_text("name", options = opt_tm_text(just="left"), col="blue", xmod=0.5)
```

In Friendly's revisiting of Guerry's work (@friendly:2007), he refers to a line across France from Saint-Malo in the North-West to the Swiss border at Geneva in the South-East, which divided France into 'France obscure' (the uneducated South-West) and 'France éclairée' (the educated North-East).  This is a famous line in French historical geography, first suggested by Dupin in 1828.  Judging by the results of a recent Google query, it still attracts attention, although for different reasons.  Looking at Figure \@ref(fig:BertLine), there seems to be  some evidence for a similar geographic division in the 1950s.
...
tm_shape(zxC) + tm_polygons(fill="Agriculture", fill.scale=tm_scale(values="brewer.yl_or_br"), fill.legend = tm_legend(position = c("left", "bottom")))
```
...
tmT <- tm_shape(zx) + tm_polygons(fill="TotDens", fill.scale=tm_scale(breaks=seq(0, 15000, 2500), values="brewer.reds"), fill.legend = tm_legend(title="", frame=FALSE, bg=FALSE, position = c("left", "bottom")))
zxPS <- zx %>% filter(!Dept %in% c("Seine", "Paris"))
tmPS <- tm_shape(zxPS) + tm_polygons(fill="TotDens", fill.scale=tm_scale(breaks=seq(0, 150, 25), values="brewer.reds"), fill.legend = tm_legend(title="", frame=FALSE, bg=FALSE, position = c("left", "bottom")))
...
tmTl1 <- tm_shape(zx) + tm_polygons(fill="TotDens", fill.scale=tm_scale(breaks=seq(0, 15000, 2500), values="brewer.reds"), fill.legend = tm_legend(title="", frame=FALSE, bg=FALSE, position = c("left", "bottom")))
tmTl2 <- tm_shape(zx) + tm_polygons(fill="lTotDens", fill.scale=tm_scale(breaks=seq(0, 10, 1),  values="brewer.reds"), fill.legend = tm_legend(title="", frame=FALSE, bg=FALSE, position = c("left", "bottom")))
