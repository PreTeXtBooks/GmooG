...
gaym1 <- tm_shape(USpsl) + tm_polygons(fill="ProStateLaw", fill.scale=tm_scale(breaks=seq(0.1,0.7,0.01), values="brewer.yl_or_br"), fill.legend = tm_legend_hide())
gaym1
```
...
tm_shape(USpsl) + tm_polygons(fill="Region", fill.scale =
tm_scale(values=colblindx), fill.legend = tm_legend(position = c("left", "bottom")))
```

```{r gayMapC, fig.asp=0.5, fig.cap = 'Support for same-sex marriage at state level using a population cartogram'}
USxt <- st_transform(USpsl, crs = 9311)
USxC <- cartogram_cont(USxt, "nS")
tm_shape(USxC) + tm_polygons(fill="ProStateLaw",  fill.scale=tm_scale(breaks=seq(0.1,0.7,0.01), values="brewer.yl_or_br"), fill.legend = tm_legend_hide())
```
